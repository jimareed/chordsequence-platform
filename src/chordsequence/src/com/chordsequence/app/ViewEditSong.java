...
public class ViewEditSong implements View, TemplateSectionHandler {
	...
	public String renderSection(String section) {
		...
		if (section.equals("body")) {
			...
			result = "<div id=\"" + section + "\" >\n";
	  		...
	  		result +=  song.toSvg(pageNum, width, app.getLinesPerPage(), false);	  		
			result += "</div>\n";
	  	}
			...
			if (pageNum < (song.numPages(app.getLinesPerPage())-1)) {
				pageNum++;
			}
			...
			title.setApp(app);
			title.setSong(song);
			title.setEditView("editsongtext");
			title.setView(name);
			...
			result = "<div id=\"" + section + "\">\n";
	  		...
	  		for (int i = 0; i < sequence.size(); i++) {
				...
				if (!(ch.startsWith("riff") || ch.startsWith("Riff"))) {
					result += "<td>";
					result += chart.renderSmall(position, ch, "html", highlight);
					chordCount++;
					result += "</td>";
				}
				if (chordCount % 8 == 0) {
	  		...
	  		if (chordCount < sequence.size()) {
		  		for (int i = 0; i < sequence.size(); i++) {
					...
					if (ch.startsWith("riff") || ch.startsWith("Riff")) {
						...
						result += tch.render(tab, "html", "small", highlight);
						chordCount++;
						result += "</td>";
					}
					if (chordCount % 8 == 0) {
			...
			result += "<td valign=\"center\"><p><a href=\"/chordsequence?view=addchord&songid=" + song.getId() + "\"><img src=\"/images/add.png\" width=\"25\" height=\"25\" /></a><a href=\"/chordsequence?action=updatechordstart&view=editchord&songid=" + song.getId() + "&chord=" + encode(chord )+ "&pagenum=" + pageNumStr + "\"><img src=\"/images/edit.png\" width=\"25\" height=\"25\" /></a></p></td>";
